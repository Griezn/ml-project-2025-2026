#!/usr/bin/env python3
"""
IMPALA CNN (15 conv layers with residual blocks)
instead of Nature DQN (3 conv layers)
"""
...
from kaz_impala_cnn_module import *
...
class TrainingWrapper(BaseWrapper):
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.randomize_distortion:
            level = random.choices([0,1,2,3,4,5], weights=[1,5,4,3,2,2])[0]
    ...
    def _apply_reward_shaping(self):
        """Add shaped rewards to accelerate learning.
        ...
        try:
            ...
            archer_list = list(base_env.archer_list)
        except AttributeError:
            return
        ...
        # Arrow near-zombie bonus (per archer)
        for archer in archer_list:
            agent_name = archer.agent_name
            if agent_name in self.rewards and zombie_list:
                for arrow in archer.weapons:
                    arx, ary = arrow.rect.centerx, arrow.rect.centery
                    for zombie in zombie_list:
                        zx, zy = zombie.rect.centerx, zombie.rect.centery
                        dist = ((arx - zx)**2 + (ary - zy)**2) ** 0.5 # Euclidian dist
                        if dist < 150:
                            self.rewards[agent_name] += 0.01 * (1.0 - dist / 150.0) 
        
        # Facing zombie bonus (per archer)
        for archer in archer_list:
            agent_name = archer.agent_name
            if agent_name in self.rewards and zombie_list:
                ax, ay = archer.rect.centerx, archer.rect.centery
                nearest = min(zombie_list, key=lambda z: ((z.rect.centerx-ax)**2 + (z.rect.centery-ay)**2)**0.5) # Find nearest zombie wit euclidian dist
                zx, zy = nearest.rect.centerx, nearest.rect.centery
                dist = ((zx-ax)**2 + (zy-ay)**2) ** 0.5
                if 0 < dist < 300:
                    # Direction to zombie (normalized)
                    dx, dy = (zx-ax)/dist, (zy-ay)/dist
                    angle_rad = math.radians(archer.angle)
                    hx, hy = math.sin(angle_rad), -math.cos(angle_rad)
                    # Dot product: 1 = perfectly facing, -1 = facing away
                    facing = dx * hx + dy * hy
                    if facing > 0.4:  # Only reward when reasonably facing toward zombie
                        proximity_factor = max(0, (300 - dist) / 300)
                        self.rewards[agent_name] += 0.03 * facing * proximity_factor
...
def make_ppo_config(env_id, max_iterations=500):
    ...
    model_config = {
        "hidden_dim": 256,           # IMPALA standard (Nature DQN has 512)
        "channels": [16, 32, 32],    # IMPALA standard channel progression
    }
